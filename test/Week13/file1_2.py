...
# addition/changes: signup_view()

# new addition/changes: new imports
...
# new addition/changes: added LoginRequiredMixin
# New and anonymous users will automatically be redirected to login page.
class StudentListView(LoginRequiredMixin, ListView):
    ...
    # Purpose of redirect_field_name:
    # redirect_field_name is the URL parameter name Django uses to store
    # the “next” page the user wanted to visit before being redirected to the login page.
    # So, once the login is successful, the user will be redirected to where they
    # originally wanted to go.

    # Now, if an anonymous user tries to open:
    ### http://127.0.0.1:8000/student/

    # Django will automatically redirect them to:
    ### http://127.0.0.1:8000/login/?next=/student/
    redirect_field_name = 'next'            # ?next=/requested/path
...
class SectionListView(LoginRequiredMixin, ListView):
    ...
    # Django will automatically redirect them to:
    ### http://127.0.0.1:8000/login/?next=/section/
    redirect_field_name = 'next'            # ?next=/requested/path
...
class EnrollmentListView(LoginRequiredMixin, ListView):
    ...
    # Django will automatically redirect them to:
    ### http://127.0.0.1:8000/login/?next=/enrollment/
    redirect_field_name = 'next'            # ?next=/requested/path


# IMPORTANT: order of mixins is very important
class StudentDetail(LoginRequiredMixin, View):
    ...
    # Now, if an anonymous user tries to open:
    ### http://127.0.0.1:8000/student-detail/

    # Django will automatically redirect them to:
    ### http://127.0.0.1:8000/login/?next=/student-detail/
    redirect_field_name = 'next'            # ?next=/requested/path
...
# ---------- CHART VIEW ----------
# new addition/changes: Given that this is a function,
# we have to use a decorator to make sure that the data is only available after login.
...
class ReportsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        # Example summaries (adjust to your models)
        ctx["students_per_section"] = (
            Section.objects
            .values("code", "name")
            .annotate(n_students=Count("section_related_name"))
...
# ===================================================================================================
# addition/changes: New imports to help us create login functionality for general users
